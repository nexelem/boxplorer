...
public final class RSSExpandedReader extends AbstractRSSReader {
	...
	List<ExpandedPair> decodeRow2pairs(int rowNumber, BitArray row)
		...
		while (true) {
			ExpandedPair nextPair = this.retrieveNextPair(row, this.pairs,
					rowNumber);
	...
	private static Result constructResult(List<ExpandedPair> pairs)
		...
		ResultPoint[] firstPoints = pairs.get(0).getFinderPattern()
				.getResultPoints();
		ResultPoint[] lastPoints = pairs.get(pairs.size() - 1)
	...
	// not private for testing
	ExpandedPair retrieveNextPair(BitArray row,
			List<ExpandedPair> previousPairs, int rowNumber)
			throws NotFoundException {
		...
		do {
			...
			pattern = this
					.parseFoundFinderPattern(row, rowNumber, isOddPattern);
			if (pattern == null) {
	...
	private boolean checkPairSequence(List<ExpandedPair> previousPairs,
		...
		for (int pos = 0; pos < previousPairs.size(); ++pos) {
			this.currentSequence[pos] = previousPairs.get(pos)
	...
	private void findNextPair(BitArray row, List<ExpandedPair> previousPairs,
	...
	DataCharacter decodeDataCharacter(BitArray row, FinderPattern pattern,
		...
		int weightRowNumber = ((4 * pattern.getValue())
				+ (isOddPattern ? 0 : 2) + (leftChar ? 0 : 1)) - 1;
