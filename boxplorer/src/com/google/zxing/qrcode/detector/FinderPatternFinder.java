...
public class FinderPatternFinder {
	...
	final FinderPatternInfo find(Map<DecodeHintType, ?> hints)
		...
		for (int i = iSkip - 1; (i < maxI) && !done; i += iSkip) {
					...
					if ((currentState & 1) == 0) { // Counting black pixels
						if (currentState == 4) { // A winner?
							if (foundPatternCross(stateCount)) { // Yes
								boolean confirmed = this.handlePossibleCenter(
										stateCount, i, j);
								if (confirmed) {
			...
			if (foundPatternCross(stateCount)) {
				boolean confirmed = this.handlePossibleCenter(stateCount, i,
						maxJ);
				if (confirmed) {
					iSkip = stateCount[0];
					if (this.hasSkipped) {
	...
	protected final boolean handlePossibleCenter(int[] stateCount, int i, int j) {
		...
		float centerI = this.crossCheckVertical(i, (int) centerJ,
				stateCount[2], stateCountTotal);
		if (!Float.isNaN(centerI)) {
			// Re-cross check
			centerJ = this.crossCheckHorizontal((int) centerJ, (int) centerI,
					stateCount[2], stateCountTotal);
			if (!Float.isNaN(centerJ)) {
				...
				for (int index = 0; index < this.possibleCenters.size(); index++) {
					...
					if (center.aboutEquals(estimatedModuleSize, centerI,
							centerJ)) {
						this.possibleCenters.set(index, center.combineEstimate(
	...
	private FinderPattern[] selectBestPatterns() throws NotFoundException {
		...
		if (startSize > 3) {
			...
			float stdDev = (float) Math.sqrt((square / startSize)
					- (average * average));

			Collections.sort(this.possibleCenters,
					new FurthestFromAverageComparator(average));

			float limit = Math.max(0.2f * average, stdDev);

			for (int i = 0; (i < this.possibleCenters.size())
					&& (this.possibleCenters.size() > 3); i++) {
		...
		if (this.possibleCenters.size() > 3) {
			...
			float average = totalModuleSize / this.possibleCenters.size();

			Collections.sort(this.possibleCenters,
					new CenterComparator(average));

			this.possibleCenters.subList(3, this.possibleCenters.size())
					.clear();
		}

		return new FinderPattern[] { this.possibleCenters.get(0),
				this.possibleCenters.get(1), this.possibleCenters.get(2) };
	}
	...
	private static final class FurthestFromAverageComparator implements
		...
		public int compare(FinderPattern center1, FinderPattern center2) {
			float dA = Math
					.abs(center2.getEstimatedModuleSize() - this.average);
			float dB = Math
					.abs(center1.getEstimatedModuleSize() - this.average);
	...
	private static final class CenterComparator implements
		...
		public int compare(FinderPattern center1, FinderPattern center2) {
			if (center2.getCount() == center1.getCount()) {
				float dA = Math.abs(center2.getEstimatedModuleSize()
						- this.average);
				float dB = Math.abs(center1.getEstimatedModuleSize()
						- this.average);
				return dA < dB ? 1 : dA == dB ? 0 : -1;
			} else {
